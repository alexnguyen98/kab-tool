...
const Home: React.FC = () => {
  ...
  // const handleInput = (e: any) => setForm((state) => ({ ...state, input: e.target.value }));
    ...
    <Layout>
      ...
      {/* <div className="p-10">
        <div className="py-5 sticky top-0 bg-white z-10 flex space-x-3">
          <button onClick={handleSave} className="text-sm border-2 border-black py-1 px-4">
            Save
          </button>
          <button onClick={handleRemoveSpace} className="text-sm border-2 border-black py-1 px-4">
            Remove space
          </button>
        </div>
        <div>
        ...
        <div>
          ...
          <textarea disabled value={filteredOutput()} className="border-2 w-full p-3" rows={4} />
        </div>
        <div className="flex items-center mb-5">
          ...
          <button onClick={handleDeleteItems} className="mt-auto ml-5 text-sm border-2 border-black py-1 px-4">
            Delete all
          </button>
        </div>
        <div>
          <h2>Input(cipher text)</h2>
          <textarea onChange={handleInput} value={form.input} className="border-2 w-full p-3" rows={4} />
        </div>
      </div> */}
    </Layout>
  );
